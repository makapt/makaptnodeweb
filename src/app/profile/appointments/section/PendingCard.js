...
export default function PendingCard({ path, item, slugifyurl }) {
  const router = useRouter();
  const rebookHandler = (item) => {
    router.push(
      `/doctors/${slugifyurl(
        item?.doctors[0]?.fullName,
        item.doctorId
      )}?type=doctor`
    );
  };
  ...
  return (
    <div className="mb-4 mx-auto bg-white p-4 md:p-6 rounded-lg drop-shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <span
          className={`font-semibold px-2 py-1 rounded-sm text-[14px] bg-[rgba(255,44,0,0.1)] text-[#ff2c00]`}
        >
          Payment pending… we’re confirming. This will auto-expire if unpaid.
        </span>
      </div>
      ...
      <div className="cursor-pointer flex md:flex-row gap-4 md:gap-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm mb-1">{item?.profiles[0]?.clinicName}</p>
          <p className="text-sm mb-2">
            <FaMapMarkerAlt className="inline-block text-red-500" />{" "}
            {item?.profiles[0]?.address.city},{" "}
            {item?.profiles[0]?.address.state}
          </p>
        </div>
      </div>
